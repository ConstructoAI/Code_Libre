/**
 * Metre PDF - CalculatorPanel : look Construction Master Pro identique au
 * mobile et au module Calculateurs ERP.
 *
 * Refonte v2 : remplace l'ancien design "metre-surface vert" par le look
 * Master Pro charcoal pillules + LCD vert pale + accent orange Conv.
 * Logique calcReducer 100% identique (importee depuis ../utils/).
 *
 * Integration metre-pdf preservee :
 *   - Modal flottant (fixed inset-0 z-50) au-dessus du PDF viewer
 *   - Header avec titre + bouton X qui appelle toggleCalculator du store
 *   - ESC ferme le panel entier (en plus des sheets internes)
 *   - panel max-w-[680px] centre + scroll vertical si necessaire
 */

import { useReducer, useState, useEffect, useRef } from 'react';
import { Clock, Trash2, Ruler, Hash, X, Calculator } from 'lucide-react';
...
export default function CalculatorPanel() {
  const toggleCalculator = useMetreStore((s) => s.toggleCalculator);
  ...
  // Auto-focus le panel au mount pour capter ESC immediatement.
  useEffect(() => {
    panelRef.current?.focus();
  }, []);

  // Keyboard support (idem mobile et MasterProCalculator).
  // ATTENTION: e.stopPropagation() pour eviter que le PDF viewer parent
  // capture les digits/operations pendant que la calc est ouverte.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // Si une sheet interne est ouverte (FtIn/Conv/Hist), deleguer entierement
      // le keyboard a la sheet -- pas de digits/operations sur la calc en
      // arriere-plan (coherent avec MasterProCalculator du module Calculateurs).
      // Exception: ESC est gere ci-dessous (ferme la sheet via son propre handler).
      if (showUnits || showHistory || showFtIn) {
        // ESC : laisser tomber, la sheet's own ESC handler s'en charge
        return;
      }

      // ESC ferme le panel entier (uniquement si pas de sheet ouverte)
      if (k === 'Escape') {
        e.preventDefault();
        e.stopPropagation();
        toggleCalculator();
  ...
  }, [state.dimMode, state.newNumber, state.dimFracNum, showUnits, showHistory, showFtIn, toggleCalculator]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 p-4">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Calculatrice Construction Master Pro"
        className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl w-full max-w-[680px] max-h-[95vh] flex flex-col outline-none focus:ring-2 focus:ring-amber-400/50"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-700 shrink-0">
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-white flex items-center gap-2">
            <Calculator className="w-4 h-4" /> Calculatrice Construction Master Pro
          </h2>
          <button
            type="button"
            onClick={toggleCalculator}
            aria-label="Fermer la calculatrice"
            className="p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 transition-colors focus-visible:ring-2 focus-visible:ring-amber-400 outline-none"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          <div className="mt-4 grid grid-cols-4 gap-2">
            ...
            <button
              ...
              aria-label="Conversion d'unités"
        ...
        {showUnits && (
          <Sheet title="Conversion d'unités" icon={<Ruler className="w-4 h-4" />} onClose={() => setShowUnits(false)}>
...
/* ── Sub-components (identiques a MasterProCalculator) ────────── */
...
function Btn({
  label, shift, onClick, tone = 'default', active, ariaPressed, ariaLabel,
}: {
  label: string;
  shift?: string;
  onClick: () => void;
...
function Sheet({
  ...
  const titleId = `metre-sheet-${title.replace(/\s+/g, '-').toLowerCase()}`;
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[70] w-full max-w-md mx-3 bg-white dark:bg-zinc-800 rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-700 p-4"
      >
...
function FtInSheet({
  isOn, onSubmit, onToggleMode, onClose,
}: {
  isOn: boolean;
  ...
  const titleId = 'metre-sheet-feet-inches';
    ...
    <>
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          >
            Mode {isOn ? 'pieds-pouces (activé)' : 'décimal'}
          </button>
          <button
            type="button"
