/**
 * ERP React Frontend - File Upload Hook
...
export function useFileUpload() {
  ...
  const upload = useCallback(async (file: File): Promise<UploadResult | null> => {
    ...
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      setError(`Le fichier depasse la taille maximale de ${MAX_FILE_SIZE_MB} Mo`);
      return null;
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Erreur lors du telechargement';
      setError(message);
      return null;
    } finally {
  ...
  const uploadMulti = useCallback(async (files: File[]): Promise<UploadResult[]> => {
    ...
    for (const file of files) {
      if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
        setError(`Le fichier "${file.name}" depasse la taille maximale de ${MAX_FILE_SIZE_MB} Mo`);
        return [];
      }
    }
