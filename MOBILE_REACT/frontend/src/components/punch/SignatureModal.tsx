/**
 * SignatureModal - Signature tactile par un superviseur externe
 *
 * L'employe pointe sa sortie, presente son telephone au superviseur sur place
 * (directeur d'usine du client, contremaitre externe, etc.) qui signe l'operation
 * (punch) du bon de travail directement sur l'ecran tactile, sans avoir a creer
 * de compte Constructo AI.
 */
...
const SignatureModal: React.FC<SignatureModalProps> = ({
  ...
  useEffect(() => {
    ...
    const initCanvas = () => {
      ...
      // Si le rect a la meme largeur/hauteur physique, ne pas recommencer (no-op).
    ...
    // fenetre est redimensionnee. La signature en cours est perdue, c'est un
    // trade-off acceptable : sans re-init, les coords tactiles seraient decalees.
    const handleResize = () => {
      initCanvas();
      setHasSignature(false);
    };
  ...
  const startDrawing = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    // Un seul doigt = dessin ; deux doigts ou plus = laisser le navigateur gerer.
    ...
    // Sauvegarde fillStyle pour ne pas perturber le contexte global.
    const prevFillStyle = ctx.fillStyle;
    ctx.beginPath();
    // Dessine un point minime pour qu'un tap court (sans drag) compte comme signature
    // valide (sinon hasSignature reste false jusqu'au premier touchmove).
  ...
  const draw = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    // Avorte si un 2e doigt rejoint : evite que le trait suive l'autre doigt.
    if ('touches' in e && e.touches.length > 1) {
      isDrawingRef.current = false;
      return;
    }
  ...
  const elapsed = context?.elapsedMinutes;
  const elapsedLabel = (() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white dark:bg-gray-800 w-full max-w-lg max-h-[95vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl p-5 space-y-4 shadow-xl">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Presentez votre telephone au superviseur sur place pour qu'il signe.
            </p>
          </div>
          <button
            onClick={onClose}
            disabled={isSubmitting}
            className="text-gray-500 hover:text-gray-700 dark:text-gray-400 shrink-0 -mt-1 -mr-1 p-1"
            aria-label="Fermer"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        {context && (context.numeroBt || context.operationNom || context.projectNom) && (
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 p-3 text-sm space-y-1">
            <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <FileText className="h-4 w-4 shrink-0" />
              <span className="font-medium truncate">
                BT {context.numeroBt ?? '--'}
                {context.operationNom ? ` - ${context.operationNom}` : ''}
              </span>
            </div>
            {context.projectNom && (
        ...
        <div>
          ...
          <input
            ...
            placeholder="ex. Jean Tremblay, directeur maintenance"
            maxLength={200}
            disabled={isSubmitting}
