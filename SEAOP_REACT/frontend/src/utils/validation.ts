/**
 * SEAOP React Frontend - Validation Functions
...
export function validatePassword(password: string): {
  ...
  if (password.length < SECURITY_CONFIG.passwordMinLength) {
    return {
      valid: false,
      message: `Le mot de passe doit contenir au moins ${SECURITY_CONFIG.passwordMinLength} caractères.`,
    };
  }
  ...
  if (!hasUpper || !hasLower || !hasDigit) {
    return {
      valid: true,
      message:
        'Mot de passe accepté. Pour plus de sécurité, utilisez des majuscules, minuscules et chiffres.',
    };
  }
...
export function validateNote(note: number): string | null {
  if (!Number.isInteger(note) || note < 1 || note > 5) {
    return 'La note doit être un entier entre 1 et 5.';
  }
  return null;
}
